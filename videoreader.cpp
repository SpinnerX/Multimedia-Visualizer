...
static std::string mediaTypeToString(AVMediaType type){
    switch (type) {
...
bool video_reader_open(VideoReaderState* state, const char* filename) {
    ...
    AVCodec* av_codec;
    printf("av_format_ctx->nb_streams value is %i\n", av_format_ctx->nb_streams);

    for (int i = 0; i < av_format_ctx->nb_streams; ++i) {
        av_codec_params = av_format_ctx->streams[i]->codecpar;
        printf("AVMEDIA TYPE value is %s\n", mediaTypeToString(av_codec_params->codec_type).c_str());
